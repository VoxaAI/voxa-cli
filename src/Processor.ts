...
export function downloadProcessor(voxaSheets: VoxaSheet[], AVAILABLE_LOCALES: string[]) {
  ...
  return voxaSheetsDownloads.map((voxaSheet: VoxaSheet) => {
    ...
    const data = _.chain(voxaSheet.data).filter(item => {
      const tempItem = _.omitBy(item, _.isEmpty);
      return !_.isEmpty(tempItem);
    }).value();
    return { name, data, locale } as Download;
  });
}
...
export function viewsProcessor(voxaSheets: VoxaSheet[], AVAILABLE_LOCALES: string[]) {
  ...
  return voxaSheetsViews.map((voxaSheet: VoxaSheet) => {
    ...
    .reduce((acc, view) => {
      ...
      const isASuggestionChip = [".dialogflowsuggestions", ".facebooksuggestionchips"].find(option => pathLowerCase.includes(option));
...
export function intentUtterProcessor(voxaSheets: VoxaSheet[], AVAILABLE_LOCALES: string[]) {
  ...
  .map(voxaSheetIntent => {
    ...
    .map((row) => {
      const info = _.pick(row, ['Intent', 'slotType', 'slotName', 'environment', 'platformIntent', 'events', 'canFulfillIntent', 'startIntent', 'signInRequired', 'endIntent']);
    ...
    .reduce((acc, item) => {
      ...
      let samples = _.chain(voxaSheetsUtter)
