...
const ALLOWED_ATTRIBUTES = ['platform', 'spreadsheets', 'speechPath', 'content', 'contentPath', 'viewsPath'];
...
export function action()  {
  ...
  const observe = Observable.create(function(obs: any) {
    ...
    const mustBeSpreadsheetURLOrLength30 = (input: string) => {
      ...
      return (noEmptyAnswer(input) && result) ? true : 'Insert a proper spreadsheet url or id eg. https://docs.google.com/spreadsheets/d/XXXXXXX/edit#gid=0';;
    }
    ...
    const questions = [
      ...
      { type: 'list', name: 'platform', message: 'Choose platform', choices: ["alexa", "dialogflow"], when: override,},
      { type: 'input', name: 'spreadsheets', message: 'Specify all spreadsheets separated by comma (*)',
        validate: mustBeSpreadsheetURLOrLength30,
        filter: onlyCommaAnswer,
        when: override,
      },
      { type: 'input', name: 'speechPath', message: 'Specify folder path to save interaction model',
        default: 'speech-assets',
        when: override,
      },

      { type: 'input', name: 'content', message: 'Specify all other sheets to download separated by comma',
        default: '',
        filter: onlyCommaAnswer,
        when: override,
      },
      { type: 'input', name: 'contentPath', message: 'Specify folder path to save all downloable content',
        when : (answers: any) => !_.isEmpty(answers.content),
        default: 'content',

      },
      { type: 'input', name: 'viewsPath', message: 'Specify folder path to save views file',
      default: '/',
      when: override,
      },
      {
  ...
  function executePrompt(): any {
    ...
    .then((answers:any) => {
      ...
      return fs.outputFileSync(path.join(process.cwd(), 'interaction.json'), JSON.stringify(answers, null, 2), { flag: 'w' });
    });
  }
